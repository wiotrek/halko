...
namespace Core
{
    public class TaskManager : ITaskManager
    {
        ...
        public async Task Run(Func<Task> function, [CallerMemberName]string origin = "", [CallerFilePath]string filePath = "", [CallerLineNumber]int lineNumber = 0)
        {
            ...
            {
                LogError(ex.ToString(), origin: origin, filePath: filePath, lineNumber: lineNumber);
                throw;
            }
        }
        ...
        public async Task Run(Action action, [CallerMemberName]string origin = "", [CallerFilePath]string filePath = "", [CallerLineNumber]int lineNumber = 0)
        {
            ...
            {
                LogError(ex.ToString(), origin: origin, filePath, lineNumber);
                throw;
            }
        }
        ...
        private void LogError( string errorMessage, [CallerMemberName] string origin = "", [CallerFilePath] string filePath = "", [CallerLineNumber] int lineNumber = 0 )
        {
            IoC.Base.IoC.Logger.Log ( $"An unexcepted error occured running a IoC.Task.Run. {errorMessage}",
                LogLevel.Debug, origin, filePath, lineNumber );
        }
